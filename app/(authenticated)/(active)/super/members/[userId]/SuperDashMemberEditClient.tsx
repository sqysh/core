...
import { SuperMemberEditData } from '@/lib/actions/user/getUserById'
import uploadFileToFirebase from '@/lib/utils/firebase/uploadFileToFirebase'
import { updateMember } from '@/lib/actions/user/updateMember'
    ...
    <div className="min-h-screen bg-bg-light dark:bg-bg-dark">
      <div className="px-6 pb-24">
        ...
        <div className="pt-7 pb-6 border-b border-border-light dark:border-border-dark mb-8 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="text-right">
              ...
              <p
                className={`text-[12px] font-mono font-bold ${member.hasAnnualSubscription ? 'text-emerald-500' : 'text-muted-light dark:text-muted-dark'}`}
              >
                {member.hasAnnualSubscription ? '✓ Active' : '✗ None'}
              </p>
            </div>
            <div className="text-right">
              <p className="text-[9.5px] font-mono tracking-[0.15em] uppercase text-muted-light dark:text-muted-dark">
              ...
              <p
                className={`text-[12px] font-mono font-bold ${member.hasQuarterlySubscription ? 'text-emerald-500' : 'text-muted-light dark:text-muted-dark'}`}
              >
                {member.hasQuarterlySubscription ? '✓ Active' : '✗ None'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                {preview && !uploading && (
                  <button
                    onClick={removeImage}
                    className="flex items-center gap-1.5 h-10 px-3 border border-red-200 dark:border-red-400/20 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-400/5 transition-colors text-f10 font-mono tracking-widest uppercase"
                  >
            ...
            <div className="flex flex-col gap-2">
              <div className="border border-border-light dark:border-border-dark px-4 py-3.5">
                <Toggle
                  ...
                  description={
                    form.isPublic ? 'Visible on the public member directory' : 'Hidden from the public member directory'
                  }
                />
              </div>
              <div className="border border-border-light dark:border-border-dark px-4 py-3.5">
                ...
                <select
                  ...
                  className="w-full h-11 px-3 bg-white dark:bg-bg-dark border border-border-light dark:border-border-dark font-nunito text-[14px] text-text-light dark:text-text-dark focus:outline-none focus:border-primary-light dark:focus:border-primary-dark transition-colors rounded-none appearance-none cursor-pointer"
                >
                ...
                <p className="text-f10 font-mono text-muted-light dark:text-muted-dark mt-2">
                  {form.role === 'ADMIN'
                    ...
                    : 'Standard access — no admin privileges'}
                </p>
              </div>
              <div className="border border-border-light dark:border-border-dark px-4 py-3.5">
                <Toggle
                  ...
                  description={form.isMembership ? 'Handles membership and billing inquiries' : 'No membership role'}
                />
              </div>
            </div>

            {/* Danger zone */}
            <div className="border border-red-200 dark:border-red-400/20">
              <div className="px-4 py-3 border-b border-red-200 dark:border-red-400/20 bg-red-50 dark:bg-red-400/5">
                <p className="text-f10 font-mono tracking-[0.2em] uppercase text-red-500 dark:text-red-400">
                  Danger Zone
                </p>
              </div>
              <div className="px-4 py-4 flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-[12px] font-nunito text-muted-light dark:text-muted-dark">
                    Permanently removes the user and all associated data. Cannot be undone.
                  </p>
                </div>
                {confirmDelete ? (
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      disabled={deleting}
                      className="h-9 px-4 bg-red-500 text-white font-sora font-bold text-sm hover:opacity-90 transition-opacity disabled:opacity-50"
                    >
                      {deleting ? 'Deleting…' : 'Yes, delete'}
                    </button>
                    <button
                      onClick={() => setConfirmDelete(false)}
                      className="h-9 px-4 border border-border-light dark:border-border-dark text-muted-light dark:text-muted-dark font-nunito text-sm hover:bg-surface-light dark:hover:bg-surface-dark transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="shrink-0 h-9 px-4 border border-red-200 dark:border-red-400/30 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-400/5 transition-colors text-f10 font-mono tracking-[0.15em] uppercase"
                  >
                    Delete
                  </button>
                )}
          ...
          {/* RIGHT — payment methods + activity */}
          <div className="space-y-8">
            {/* Payment methods */}
            <div>
              ...
              {member.paymentMethods.length === 0 ? (
                <div className="border border-border-light dark:border-border-dark px-4 py-6 text-center">
                  <p className="text-[11px] font-mono text-muted-light dark:text-muted-dark">No card on file</p>
                </div>
              ) : (
                <div className="flex flex-col gap-2">
                  {member.paymentMethods.map((pm) => (
                    <div
                      key={pm.id}
                      className={`border px-4 py-3.5 flex items-center justify-between gap-3 ${pm.isDefault ? 'border-primary-light/30 dark:border-primary-dark/30 bg-primary-light/5 dark:bg-primary-dark/5' : 'border-border-light dark:border-border-dark'}`}
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-[11px] font-mono font-bold text-text-light dark:text-text-dark uppercase tracking-widest">
                          {pm.brand}
                        </span>
                        <span className="text-[12px] font-mono text-muted-light dark:text-muted-dark">
                          •••• {pm.last4}
                        </span>
                      ...
                      <div className="text-right">
                        {pm.isDefault && (
                          <span className="text-[9px] font-mono tracking-widest uppercase px-1.5 py-0.5 border border-primary-light/30 dark:border-primary-dark/30 text-primary-light dark:text-primary-dark">
                            Default
                          </span>
                        )}
              ...
              <div className="flex border-b border-border-light dark:border-border-dark mb-4">
                {(['meetings', 'referrals', 'closed'] as const).map((tab) => (
                  <button
                    ...
                    className={`h-9 px-4 text-f10 font-mono tracking-[0.15em] uppercase border-b-2 transition-colors ${
                      activeTab === tab
                  ...
                  >
                    {tab === 'meetings'
                      ? `F2F (${member.activity.face2face.length})`
                      : tab === 'referrals'
                        ? `Refs (${member.activity.referrals.length})`
                        : `Closed (${member.activity.tyfcb.length})`}
                  </button>
                ))}
              </div>

              <div className="border border-border-light dark:border-border-dark divide-y divide-border-light dark:divide-border-dark max-h-150 overflow-y-auto">
                {activeTab === 'meetings' && (
                  <>
                  ...
                  <>
                    ...
                    {member.activity.referrals.map((r) => (
                      <div key={r.id} className="flex items-center gap-3 px-4 py-3">
                        <span className="w-2 h-2 rounded-full bg-secondary-dark shrink-0" />
                        <div className="flex-1 min-w-0">
                    ...
                    {member.activity.tyfcb.map((a) => (
                      <div key={a.id} className="flex items-center gap-3 px-4 py-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11.5px] font-nunito text-muted-light dark:text-muted-dark truncate">
                            {a.giverId === member.id ? `→ ${a.receiver?.name}` : `← ${a.giver?.name}`} · {a.description}
                          </p>
                        </div>
                        <div className="text-right shrink-0">
      ...
      {/* ── Save bar ── */}
      <div
        className="fixed bottom-0 left-0 right-0 bg-bg-light dark:bg-bg-dark border-t border-border-light dark:border-border-dark px-6 py-3 z-40"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.back()}
            className="h-11 px-5 border border-border-light dark:border-border-dark text-muted-light dark:text-muted-dark font-nunito text-sm hover:bg-surface-light dark:hover:bg-surface-dark transition-colors inline-flex items-center"
          >
            Cancel
          </button>
          <button
            ...
            className="h-11 px-8 bg-primary-light dark:bg-button-dark text-white font-sora font-bold text-sm tracking-wide hover:opacity-90 active:opacity-80 transition-opacity disabled:opacity-50"
          >
